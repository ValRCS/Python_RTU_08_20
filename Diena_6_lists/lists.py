...
print(my_numbers[::3]) # nums starting with 0, 3, 6, 9
...
print("fox" in words)
words[3] = words[3].upper() # i can do something with smaller elements in a list
...
new_list.extend(["Fantastiskais", "Lapsa"]) # very similar to += IN PLACE, so call flat
print(new_list)

print(new_list[-3])
print(new_list[-3][0], new_list[12][0], new_list[14][0])
...
print(str_list == str_list_2) # check if 1st level values are equal
print(str_list is str_list_2) # check if values are same object (reside in the same memory space), here False

print(my_numbers)
squares = [n**2 for n in my_numbers] # go through all my numbers and square them
print(squares)
...
odd_squares_2d = [[n, n*n] for n in my_numbers if n%2 == 1] # so do something and also filter
