...
# # # # # # # # # # # # # # # # key - value pairs
# # # # # # # # # # # # # # # # unordered - unlike lists, dictionaries are not ordered
...
empty_dict = {}
another_empty_dict = dict()  #so do not use dict as a variable!
# # # # # # 
print(empty_dict)
print(another_empty_dict)

# print length of empty_dict
print("Length of dictionary", len(empty_dict))
print(type(empty_dict))

tel_dict = {"valdis": 2640, "līga": 2911}  # so dictionary initialize with 2 key:value pairs
print(tel_dict)
tel = {'valdis': 2640, 'līga': 2911}  #also a dictionary
print(tel)
print("Contents same", tel_dict == tel)
print("Are actually same data structure in memory:", tel_dict is tel) # two different data structures in memory

# # # # get value by key
print(tel['valdis'])  # so get value from key, very fast(O(1) even with millions and billions of items)

# # # # add new key - value pair or overwrite the old one
...
# i can provide key through variable
key = "maija" # instead of key it could be any variable
print(tel[key], tel["maija"])  # identical results
...
tel['valdis'] = [2640, 1188, 911]  # pylance and pyright will warn of type change
print(tel)
print(tel['valdis']) # i get a list
print(tel['valdis'][-1], tel['valdis'][2])  # normal list operations
# the above will not work for maija, because maija has only one phone number not a list
...
# # # # # # # # # # tel['valdis'] = 2640
# # # # # # # # # # print(tel)
tel['petēris'] = 2911  # any string can be key, values can be duplicated
print(tel)
...
# # # just the keys 
print(tel.keys())
print(type(tel.keys())) # so this is a dict_keys object looks like a list a bit
key_list = list(tel.keys()) # so i can create a list of dictionary keys
print(key_list) # not related any more to dictionary

# # # # # # # # # # # similarly I can extract values from a dictionary
value_list = list(tel.values())
print(value_list)

# # so now key_list and value_list have no relation to dictionary, just two independent lists
...
print(new_dict)
print(tel)
# reversed_dict = dict(zip(value_list, key_list)) # so keys will be integers and values will be strings
# print(reversed_dict)
# now we can reverse the dictionary
reversed_dict = dict(zip(tel.values(), tel.keys())) # so keys will be integers and values will be strings
print(reversed_dict)

print(reversed_dict[1001])  # this is a dictionary lookup not a list lookup!
# numeric keys will be useful if you have large gaps in numbers
# for example if you have 1000 phone numbers and you want to store them in a dictionary
# you can use the phone number as a key and the value can be a list of names
# if values are small and close together you can use a list instead of a dictionary

# # # # # # # # fast checkup on key existance
try:
    print(tel["nevaldis"]) # this will give an error
except KeyError:
    print("Sorry no nevaldis in our dictionary")

print('valdis' in tel) # so i check for key in my dictionary very quickly
print('ValdisZ' in tel) # case sensitive so True
print('Valdis' in tel) # case sensitive so False
print('joker' in tel)

# checking existence of value is a bit more complicated and slower
# # # # # # # # this will take O(n) operations on n sized dictionary
print(1001 in tel.values()) # this will be slow in large dictionaries
...
# i could store another dictionary inside a dictionary
...
# # print(tel['badkey']) # this will be an error I could use try except but there is another way
# my_key = "valdis"
my_key = "notarealkey"  # overwriting the previous my_key value
if my_key in tel:
    # do something with the key including changing the value
    print("key:", my_key, "value:", tel[my_key])
else:
    print("Couldn't find this key", my_key)

# # # # # so dictionary get method is a way of handling bad keys
print(tel.get('valdis')) # so we get value by key 'valdis' similar to tel['valdis'] but without errors
print(tel.get('nevaldis'))  # default is None if not found, but no error
...
for key in tel: # so we can iterate over all keys in a dictionary, key is just a variable name
...
print("Done looping through dictionary")
...
for key, value in tel.items(): # also common is k,v, but could name them anything
    ...
    print(f"key={key}, value={value}, {tel[key]}")  # from Python 3.6+
# # # # # # #     # do more stuff with each key value pair
# # # # # # # # # # # print(tel.items())
# # # # # # # # # # # # # print(tel)

tel_copy = tel.copy()  # so called shallow copy - deeper references are not copied
...
for key, value in tel.copy().items(): # also common is k,v
    ...
    if key == "valdis":
        tel["jaunvaldis"] = 1900 # safe to add new key only if going through copy
#     # we add new keys to our dictionary and it is safe because we are looping through copy
#     # tel[key*2] = value # so we can modify original dictionary size

print(tel)

# # # # # # # # # # # # # # # idea to set key value pair UNLESS it is already set
print(tel)
tel.setdefault("rūta", 2911) # so setdefault only works once per key
print(tel)
...
new_list = [] # empty list to hold just the names/keys
...
for key, value in tel.items():  #no need for copy() since I am modifying different dict
    if value == value_to_find:
        ...
        new_list.append(key) # saving just the names in a list

print(new_dict)
print(new_list, list(new_dict.keys()))

# # # # # # # # # # # # # # delete
del tel["my_dict"]  # in place modifies the dictionary
print(tel)
del tel['petēris'] # will throw error if key is not found
print(tel)
try:
    del tel['petēris'] # will throw error if key is not found
except KeyError:
...
# # print(tel)
popped_value = tel.pop("Valdisz") # so we can get the value that was deleted
print(tel)
print(popped_value)
print(tel.pop("empty_string", "my_default"))
# tel.pop("no_such_key", "my_default") # we can use this to delete a key if it is not there

# # # # # # # # # # how to delete multiple keys from dict
bad_key_list = ["none_value", "many_values", "Valdisz", "nosuchkey","my_dict","", "empty_string"]
for bad_key in bad_key_list: # we are not going through our dictionary, so it is okay to delete
    if bad_key in tel:
        print("Deleting", bad_key)
        del tel[bad_key]
    else:
        print("Nothing to delete did not find", bad_key)
...
no_aldis_dict = {k+" S":v*10_000 for k,v in tel.items() if "aldis" not in k} # if is not required
print("Dict comprehension", no_aldis_dict)
...
# some_name = "Valdis Saltonis"
# word_dict = {k:ord(k) for k in some_name}  # so dictionary of letters -> unicode values
# print(word_dict)
...
# tel.clear() # IN PLACE deletes everything in dictionary
# print(tel)
...
# boxes = "ABCDE"
# box_dict = {box:0 for box in boxes}
# print(box_dict)
# box_dict["C"] += 15 # same as box_dict["C"] = box_dict["C"] + 15
# print(box_dict)
# box_dict["C"] += 13
# print(box_dict)
...
# letter_dictionary = {k:ord(k) for k in letters } # so dictionary of letters and their Unicodes
...
# # # ede = tel.pop('Ede', "couldnt find this key") # ede does not exist so ede will have default value
# # # print(ede)
# # # ruta = tel.pop("Rūta", "no such key")
# # # print(ruta)
print(tel)

tel.update({"valdis":432525, "Valdis":29000, "3": 500, "foo": 5454}) # so merging dictionaries
print(tel)
...
# # print("tel is tel_alias in memory", tel is tel_alias) # so tel and tel_alias are both shortcuts to same dictionary
# # tel["ede"] = 9000
...
# # print(tel)  # tel remains unchanged since copy created a new dictionary
# # print(shallow_copy)
...
# # # OUT OF PLACE returns a new dictionary
...
def get_filtered_dict_2(in_dict, value_to_find):
    return {k:v for k,v in in_dict.items() if v == value_to_find} # i create and return a new dictionary
...
# # # # # # could help with first exercise
numbers = list(range(12))
numbers.extend([0,1,0,5,3])
print(numbers)
new_dict = {}
for n in numbers:
    if n > 4:
        ...
        if "under5" in new_dict:
            ...
            new_dict["under5"] = 1
        if n == 0:
            if "zero_count" in new_dict:
                new_dict["zero_count"] +=1
            else:
                new_dict["zero_count"] = 1   
print(new_dict)

food_list_with_duplicates = [
    "potato", 
    "banana",
    "potato",
    "potato",
...
# now i can use dictionary to count how many times each item appears
food_dict = {}
for food in food_list_with_duplicates:
    if food in food_dict:
        food_dict[food] += 1
    else:
        food_dict[food] = 1 # so set initial count to 1
# this counting approach is faster than using list count method for all items

print(food_dict)

 # # # # for counting items we also have Counter class